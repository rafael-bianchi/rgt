...
pub fn detect_and_configure_hooks(global: bool, force: bool) -> io::Result<Vec<String>> {
    ...
    let claude_dir = if global { home.join(".claude") } else { PathBuf::from(".claude") };
    ...
    let cursor_dir = if global { home.join(".cursor") } else { PathBuf::from(".cursor") };
    ...
    let codex_dir = if global { home.join(".codex") } else { PathBuf::from(".codex") };
    ...
    let windsurf_dir = if global { home.join(".codeium").join("windsurf") } else { PathBuf::from(".windsurf") };
