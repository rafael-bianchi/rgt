...
fn test_query_provenance_response_schema() {
    ...
    assert!(res.get("total_ancestors").is_some(), "missing total_ancestors");
    assert!(res.get("lineage_steps").is_some(), "missing lineage_steps");

    let steps = res.get("lineage_steps").unwrap().as_array().unwrap();
    assert!(!steps.is_empty());
    let step = &steps[0];
    assert!(step.get("node_type").is_some(), "missing node_type in lineage step");
    assert!(step.get("line_number").is_some(), "missing line_number in lineage step");
