...
- **SC-001**: A developer can record values from a data file and see them in `rgt status` within 1 second of running `rgt record`. Bulk inserts are wrapped in a single transaction; the CI test threshold is 2 seconds to account for slower shared CI runners.
