...
- **SC-001**: A developer can record values from a data file and see them in `rgt status` within 1 second of running `rgt record` (CI test threshold allows up to 5 seconds to account for slower shared CI runners; local performance target is sub-second).
